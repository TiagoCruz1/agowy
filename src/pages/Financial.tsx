...
import { TrendingUp, Users, XCircle, Clock, DollarSign, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Financial() {
  ...
  const { data: monthData, isLoading } = useQuery({
    queryKey: ["financial", userId, monthStart],
    queryFn: async () => {
  ...
  const { data: yearlyData } = useQuery({
    queryKey: ["financial-yearly", userId, currentMonth.getFullYear()],
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("appointments")
        .select("start_at, status, price")
        .eq("user_id", userId!)
        .gte("start_at", yearStart)
        .lte("start_at", yearEnd);
      if (error) throw error;

      const byMonth: Record<number, { revenue: number; completed: number; cancelled: number; noShow: number; total: number }> = {};
      for (let m = 0; m < 12; m++) {
        byMonth[m] = { revenue: 0, completed: 0, cancelled: 0, noShow: 0, total: 0 };
      }
      (data || []).forEach((a: any) => {
  ...
  const { data: studioRevenue = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const profile of profiles || []) {
        ...
        const revenue = (appointments || []).reduce((sum: number, appointment: any) => sum + (Number(appointment.price) || 0), 0);

        rows.push({
          user_id: profile.user_id,
          full_name: profile.full_name,
          is_active: links.find((link: any) => link.manicure_user_id === profile.user_id)?.is_active ?? true,
          revenue,
  ...
  const studioStaffRevenue = studioRevenue.reduce((sum, staff) => sum + staff.revenue, 0);
      ...
      {isStudioOwner && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="border-primary/20 bg-primary/5 md:col-span-1">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatBRL(monthData?.revenue || 0)}</div>
              <p className="text-xs text-muted-foreground">Concluídos no mês atual</p>
            </CardContent>
          </Card>
